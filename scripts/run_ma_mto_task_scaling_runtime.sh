...
RESULT_DIR="./results"
RESULT_FILE="$RESULT_DIR/ma_mto_task_scaling_runtime.$TIME.$PID.csv"
mkdir -p $RESULT_DIR
...
for num_tasks in 50 150 500 1500 5000; do
    for run_id in {1..3}; do
        ...
        CUDA_VISIBLE_DEVICES="$TMUX_SESSION" taskset -c "$CPU_LIST" \
            python ./experiments/ma_mto_task_scaling_runtime.py --num_tasks $num_tasks --problem_realization parallel --device cuda $header_arg 
    done

    for run_id in {1..3}; do
        CUDA_VISIBLE_DEVICES="$TMUX_SESSION" taskset -c "$CPU_LIST" \
            python ./experiments/ma_mto_task_scaling_runtime.py --num_tasks $num_tasks --problem_realization matrix --device cuda --no_headers
    done
    echo
done
