...
RESULT_DIR="./results"
RESULT_FILE="$RESULT_DIR/b_spline_trajectory_runtime_evaluation.$TIME.$PID.csv"
mkdir -p $RESULT_DIR
...
for resolution in 60 200 600 2000 6000; do
    for run_id in {1..3}; do
        ...
        CUDA_VISIBLE_DEVICES="$TMUX_SESSION" taskset -c "$CPU_LIST" \
            python ./experiments/b_spline_trajectory_runtime_evaluation.py --dimension 60 --problem_resolution "$resolution" --problem_realization parallel --device cuda $header_arg 
    done

    for run_id in {1..3}; do
        CUDA_VISIBLE_DEVICES="$TMUX_SESSION" taskset -c "$CPU_LIST" \
            python ./experiments/b_spline_trajectory_runtime_evaluation.py --dimension 60 --problem_resolution "$resolution" --problem_realization pointwise --device cuda --no_headers
    done
    echo
done
