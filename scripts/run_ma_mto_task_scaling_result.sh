...
RESULT_DIR="./results"
RESULT_FILE="$RESULT_DIR/ma_mto_task_scaling_result.$TIME.$PID.csv"
mkdir -p $RESULT_DIR
...
for seed in {0..9}; do
    ...
    CUDA_VISIBLE_DEVICES="$TMUX_SESSION" taskset -c "$CPU_LIST" \
        python ./experiments/ma_mto_task_scaling_result.py --seed $seed --num_tasks 5000 --device cuda $header_arg 
done
