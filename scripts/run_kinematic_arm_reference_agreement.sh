...
RESULT_DIR="./results"
RESULT_FILE="$RESULT_DIR/kinematic_arm_reference_agreement.$TIME.$PID.csv"
mkdir -p $RESULT_DIR
...
for seed in {0..9}; do
    ...
    CUDA_VISIBLE_DEVICES="$TMUX_SESSION" taskset -c "$CPU_LIST" \
        python ./experiments/kinematic_arm_reference_agreement.py --seed $seed --num_tasks 5000 --problem_realization parallel --device cuda $header_arg
    echo
    CUDA_VISIBLE_DEVICES="$TMUX_SESSION" taskset -c "$CPU_LIST" \
        python ./experiments/kinematic_arm_reference_agreement.py --seed $seed --num_tasks 5000 --problem_realization matrix --device cuda --no_headers
    echo
done
